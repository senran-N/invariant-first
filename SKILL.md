...
description: 构建优先的跨 harness 工程技能路由器。用于从零开发、开发者预览、功能迭代、架构与接手、代码审查、发布运行、修复、性能优化、重构迁移、产品转向、退役和文档。按当前交付目标选择 PRIMARY，按真实代码事实补充架构、契约、状态、依赖、交互、失速恢复或跨会话接手技能；需要时衔接下一条路线。以清楚的表示与责任完成授权工作，用真实反馈校正判断，让成果可用、可理解、可接续；具体方法按需加载。
---

# Invariant First

识别目标与条件，找到责任，完成工作，用反馈校正。共同判断只读一次，具体方法按需读取；运用判断，不逐条表演流程。
